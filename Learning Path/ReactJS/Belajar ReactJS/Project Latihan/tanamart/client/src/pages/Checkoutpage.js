...
function Checkoutpage() {
  ...
  const makePayment = (token) => {
    ...
    axios
      .post(`http://localhost:5000/checkout`, body, { withCredentials: true })
      .then((response) => {
  ...
  useEffect(() => {
    console.log(idUser);
    axios
      .get(`http://localhost:5000/showCart/${idUser}`)
      .then((response) => {
  ...
  const deleteHandler = (id_cart) => {
    swal({
      ...
      if (willDelete) {
        axios
          .delete(`http://localhost:5000/deleteCart/${id_cart}`)
          .then((response) => {
            console.log(response);
            ...
            <div className="cart-body py-5">
              {listOrder.map((list) => (
                <div className="d-flex justify-content-center align-items-center">
                  <div className="col-5">
                    <img
                      src={`http://localhost:5000/uploads/${list.product.foto}`}
                      alt="product"
                      className="pb-2"
                      height="300px"
                      width="300px"
