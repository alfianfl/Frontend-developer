...
// });
const search = document.querySelector('.button-search');
search.addEventListener('click', async function(){ // tambah async supaya asynchronous
    try{
        const input = document.querySelector('.input-keyword');
        const movie = await getMovie(input.value); // tambah await pada fungsi yg akan di asynchronous
        updateUI(movie);
